...
def paddle_ocr(frame, x1, y1, x2, y2):
    ...
    for r in result:
        # Access rec_texts and rec_scores from the dictionary
        rec_texts = r.get('rec_texts', [])
        rec_scores = r.get('rec_scores', [])
        
        # Iterate through detected texts and their scores
        for detected_text, score in zip(rec_texts, rec_scores):
            ...
            if score > 60:
                text = detected_text
                break  # Take the first high-confidence result
        
        if text:  # If we found a good result, stop processing
            break
